...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  if (!hash_equals($csrfToken, $postedToken)) {
    ...
    if ($_SESSION[$rateKey]['count'] >= $maxAttempts) {
      ...
      if ($stmt === false) {
        error_log('Student login prepare failed: ' . $conn->error);
        ...
        // Constant-time style verification (dummy hash if user not found)
          ...
          <form method="POST" class="space-y-5" onsubmit="return handleSubmit(this)">
            ...
            <div class="relative">
              ...
              <input id="username" type="text" name="username" required autocomplete="username"
                     class="w-full pl-3 pr-3 py-3 rounded-xl border border-blue-900/10 bg-white/70 placeholder:text-blue-900/40 text-blue-900 shadow-inner focus:outline-none focus:ring-4 focus:ring-blue-500/20 focus:border-blue-500 transition"
            ...
            <div class="relative">
              ...
              <input id="password" type="password" name="password" required autocomplete="current-password"
                     class="w-full pl-3 pr-12 py-3 rounded-xl border border-blue-900/10 bg-white/70 placeholder:text-blue-900/40 text-blue-900 shadow-inner focus:outline-none focus:ring-4 focus:ring-blue-500/20 focus:border-blue-500 transition"
              ...
              <button type="button" onclick="togglePassword()" class="absolute right-3 top-1/2 -translate-y-1/2 text-blue-900/50 hover:text-blue-900/80 transition" aria-label="Toggle password visibility">
                <svg id="eyeIcon" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 24 24" fill="currentColor"><path d="M12 5c-7 0-11 7-11 7s4 7 11 7 11-7 11-7-4-7-11-7zm0 12a5 5 0 110-10 5 5 0 010 10z"/></svg>
              </button>
            </div>
            ...
            <p class="text-center text-sm text-blue-900/70">
              Not a <?= htmlspecialchars($PORTAL_NAME) ?>?
              <a href="teacher_login.php" class="text-blue-800 font-semibold hover:underline">Teacher login</a> ·
              <a href="admin_login.php" class="text-blue-800 font-semibold hover:underline">Admin login</a>
            </p>
          </form>
        </div>
      </div>
